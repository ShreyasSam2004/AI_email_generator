...
class MITTechReviewArticle(BaseModel):
    title: str
    description: str
    url: str
    guid: str
...
class MITTechReviewScraper:
    ...
    def get_articles(self, hours: int = 24) -> List[MITTechReviewArticle]:
        ...
        for entry in feed.entries:
            ...
            if published_time >= cutoff_time:
                ...
                ai_keywords = ["ai", "artificial intelligence", "machine learning", "deep learning",
                              ...
                              "google ai", "microsoft ai", "automation", "robotics"]

                is_ai_related = any(kw in title or kw in description or kw in " ".join(tags)
                                   for kw in ai_keywords)

                if is_ai_related:
                    articles.append(MITTechReviewArticle(
...
if __name__ == "__main__":
    scraper = MITTechReviewScraper()
    articles = scraper.get_articles(hours=168)  # Last week
    print(f"Found {len(articles)} AI-related articles from MIT Technology Review")

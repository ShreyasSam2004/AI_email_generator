...
class YouTubeScraper:
    ...
    def get_transcript(self, video_id: str) -> Optional[Transcript]:
        try:
            ...
            print(
                f"Unexpected error for {video_id}: "
                f"{type(e).__name__}: {e}"
            )
            return None
...
if __name__ == "__main__":
    ...
    print("Testing single video transcript:")
    transcript = scraper.get_transcript("jqd6_bbjhS8")

    if transcript:
        print(f"Got transcript with {len(transcript.text)} characters")
    ...
    for video in channel_videos:
        ...
        print(f"{has_transcript} {video.title} ({video.video_id})")

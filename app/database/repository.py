...
class Repository:
    ...
    def bulk_create_sciencedaily_articles(self, articles: List[dict]) -> int:
        new_articles = []
        for a in articles:
            existing = self.session.query(ScienceDailyArticle).filter_by(guid=a["guid"]).first()
            if not existing:
                new_articles.append(ScienceDailyArticle(
                    guid=a["guid"],
                    title=a["title"],
                    url=a["url"],
                    published_at=a["published_at"],
    ...
    def get_articles_without_digest(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        ...
        venturebeat_articles = self.session.query(VentureBeatArticle).all()
        ...
        sciencedaily_articles = self.session.query(ScienceDailyArticle).all()
        for article in sciencedaily_articles:
            key = f"sciencedaily:{article.guid}"
            if key not in seen_ids:
                articles.append({
                    "type": "sciencedaily",
                    "id": article.guid,
